...
<form name="os" action="processa-os.php?acao=descontopeca" method="post">
    ...
    <tr>
  	...
  	<tr>
        <td>
            <?php
                include "conexao.php";
                $sqlItem = $con->prepare("SELECT * FROM TBL_ITEM_PECA_OS WHERE NUM_ID_PECA_OS = '$item_peca'");
                $sqlItem->execute();
            ...
            ?>        
                ...
                <div class="form-row">
                    ...
                    <input title="VALOR DA PECA" name="valor" id="valor" value="<?php echo $rowItem->VAL_VALOR_PECA_OS ?>"  class="form-control" readonly /> </div> 

                    <div class="form-group col-md-2 col-sm-6"><label>Desconto</label>
                    <input title="DESCONTO DA PECA" name="desconto" id="desconto" value="<?php echo $rowItem->VAL_DESCONTO_PECA_OS ?>" class="form-control" onblur="calculaDesconto()" /> </div>
                    
                    <div class="form-group col-md-2 col-sm-6"><label>Total</label>
                    <input title="VALOR FINAL DA PECA" name="total" id="total" value="<?php echo $rowItem->VAL_FINAL_PECA_OS ?>" class="form-control" readonly /> </div> 
